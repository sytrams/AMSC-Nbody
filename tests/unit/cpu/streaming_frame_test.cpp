...
TEST_F(StreamingFrameTest, WritesTypedFrameWithTotalStepCount) {
  ...
  const auto header = nbody::streaming::readFrameHeader(frame);
  EXPECT_EQ(header.version, nbody::streaming::kLegacyTypedFrameVersion);
  EXPECT_EQ(header.headerBytes,
            nbody::streaming::kLegacyTypedFrameHeaderBytes);
  ...
  EXPECT_EQ(nbody::streaming::readFrameParticleTypes(frame),
            (std::vector<std::uint8_t>(types.begin(), types.end())));
}
