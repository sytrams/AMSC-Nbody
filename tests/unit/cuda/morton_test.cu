...
TEST_F(MortonKeysTest, MapsOppositeInputCornersConsistentlyWithPaddedBoundingBox)
{
    ...
    ASSERT_EQ(actual.indices.size(), 2u);
...
TEST_F(MortonKeysTest, RejectsEmptyOrNullInput)
{
    ...
    EXPECT_THROW(
        MortonKeys(nullptr, nonNull, nonNull, 1),
        std::invalid_argument);

    EXPECT_THROW(
        MortonKeys(nonNull, nullptr, nonNull, 1),
        std::invalid_argument);

    EXPECT_THROW(
        MortonKeys(nonNull, nonNull, nullptr, 1),
        std::invalid_argument);
}
