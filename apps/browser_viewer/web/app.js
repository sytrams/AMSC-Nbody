(() => {
  ...
  function parseFrame(buffer) {
    ...
    const validHeader =
      (version === LEGACY_VERSION && headerBytes === LEGACY_HEADER_BYTES) ||
      (version === QUANTIZED_VERSION && headerBytes === QUANTIZED_HEADER_BYTES) ||
      (version === TYPED_VERSION && headerBytes === TYPED_HEADER_BYTES);
    if (!validHeader || buffer.byteLength < headerBytes) {
      throw new Error(`Unsupported frame format version ${version}`);
    }
    ...
    const payloadBytes = safeNumber(view.getBigUint64(64, true), "Frame payload");
    ...
    let bounds = null;
    if (version >= QUANTIZED_VERSION) {
      bounds = {
        minimum: [view.getFloat64(72, true), view.getFloat64(80, true), view.getFloat64(88, true)],
        maximum: [view.getFloat64(96, true), view.getFloat64(104, true), view.getFloat64(112, true)],
      };
    }
    const particleTypeBytes =
    ...
    if (version === LEGACY_VERSION) {
      if (scalarBytes !== 4) {
        throw new Error("The legacy frame scalar width is invalid");
      }
