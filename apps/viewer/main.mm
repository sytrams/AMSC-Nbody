...
ViewerOptions parseOptions(int argc, char **argv) {
  ...
  if (options.file && options.streamDirectory)
    throw std::invalid_argument(
        "--file and --stream-dir cannot be used together");
  if (!options.file && !options.streamDirectory)
...
- (void)poll:(NSTimer *)timer {
  ...
  @autoreleasepool {
    try {
      const std::optional<StreamState> state = readStreamState(_directory);
      ...
      _streamEnded = state && state->ended;

      std::optional<fs::path> frame = markerSnapshot(_directory);
...
int runViewer(const ViewerOptions &options) {
  @autoreleasepool {
    ...
    if (options.file) {
      ...
      fs::create_directories(*options.streamDirectory);
      delegate.watcher = [[FrameWatcher alloc]
          initWithDirectory:*options.streamDirectory
                    renderer:renderer
                      window:window
...
int validate(const ViewerOptions &options) {
  ...
  if (options.file) {
    ...
    std::optional<fs::path> latest = markerSnapshot(*options.streamDirectory);
    if (!latest)
      latest = newestSnapshot(*options.streamDirectory);
    if (!latest)
      throw std::runtime_error("No complete .nbsnap frame is available in " +
                               options.streamDirectory->string());
    path = *latest;
  }
