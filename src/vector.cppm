...
public:
    ...
    //operators
    VectorBase &operator=(std::nullptr_t){ // set all components to zero
    ...
    VectorBase &operator=(VectorBase &&other) noexcept { // move all values to this
        for (std::size_t i = 0; i < DIM; ++i)
    ...
    inline friend std::ostream &operator<< (std::ostream &stream, const VectorBase &vector){ // print vector to output stream
        stream << "{";
...
public:
    ...
    Vector(const double value) : VectorBase(std::array<double, 2>{value, value}) {}
...
public:
    ...
    Vector(const double value) : VectorBase(std::array<double, 3>{value, value, value}) {}
